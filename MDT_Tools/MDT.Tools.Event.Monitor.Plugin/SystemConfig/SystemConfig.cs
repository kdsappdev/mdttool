...
namespace MDT.Tools.Event.Monitor.Plugin
{
    public class SystemConfig
    {
        ...
        private static string GetWorkDirectory()
        {
            try
            {
                return Path.GetDirectoryName(typeof(SystemConfig).Assembly.Location) + "\\..\\control";
            }
            catch
            {
                return System.Windows.Forms.Application.StartupPath + "\\..\\control";
            }
        }
        /// <summary>
        /// 判断字符串是否为空串
        ...
        #region"配置文件的读取和写入"
        private static string CONFIG_FILE = "EventSystemConfig.xml";
        /// <summary>
        ///  读取指定的配置文件中指定Key的值
        /// </summary>
