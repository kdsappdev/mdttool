...
namespace NVelocity.Runtime.Log {
	...
	private void  internalInit(String logfile) {
	    ...
	    // create the category
	    this.logger = domain.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.FullName);
	}
	...
	public virtual void shutdown() {
	    if (logger != null) {
		if (((Logger)logger).GetAllAppenders() != null) {
		    IEnumerator appenders = ((Logger)logger).GetAllAppenders().GetEnumerator();
		    while (appenders.MoveNext()) {
			IAppender appender = (IAppender) appenders.Current;
			appender.Close();
		    }
		}
	    }
