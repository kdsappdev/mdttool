...
namespace Atf.Installer.SetupCheck.Util
{
    public class Fonts
    {
        ...
        public int callback1(ref ENUMLOGFONTEX lpelfe, ref NEWTEXTMETRICEX lpntme, int FontTypeParam, int lParam)
        {
            try
            {
                FontType fontType;
                if (FontTypeParam == 2 || FontTypeParam == 4)
                {
                    fontType = (FontType)FontTypeParam;
                }
                else
                {
                    fontType =FontType.Unknown;
                }
