...
static void initBase64DecodeTable()
{
  ...
  char* base64Decode(char* in, unsigned int& resultSize, bool trimTrailingZeros) 
{
  ...
  if (!haveInitedBase64DecodeTable)
  {
    ...
    char* out = (  char*)strDupSize(in); // ensures we have enough space
  int k = 0;
  ...
  for (int j = 0; j < jMax; j += 4) 
  {
	...
	{
      inTmp[i] = in[i+j];
      outTmp[i] = base64DecodeTable[(  char)inTmp[i]];
