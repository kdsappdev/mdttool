...
namespace MDT.Tools.Server.Monitor.Plugin
{
    public class TextLogHelper : ILog
    {
        ...
        public void Info(string info)
        {
            ...
            if("CONNECTION_LOGIN_SUCCESS".Equals(info))
            {
                SRControl.setTimeEnable(true);
            }
        }

        public void Debug(string str)
        {
            SetInfo(string.Format("地址：{2}:{3}  {0}  {1}", DateTime.Now.ToString(DateTimeFormat), "", ip, port));
        ...
        private void SetInfo(string info)
        {
            ...
            if (_txtBox!=null&&!_txtBox.IsDisposed)
            {
